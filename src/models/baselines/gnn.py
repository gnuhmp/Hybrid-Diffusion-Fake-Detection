...
class Model(torch.nn.Module):
    def __init__(self, in_channels, nhid, num_classes=2, model_type='sage', concat=False, dropout_ratio=0.0):
    ...
    def forward(self, data):
        ...
        if self.concat:
            news = torch.stack([data.x[(data.batch == idx).nonzero().view(-1)[0]] for idx in range(data.num_graphs)])
